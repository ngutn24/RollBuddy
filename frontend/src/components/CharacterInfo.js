    ...
    return (
        <Row>
            <Col lg={3} style={charDetails}>
                <h1>Name:</h1>
                <input value={name}
                       onChange={updateStateHandler(setName)}
                       style={longInputStyle}
                />
                <h3>Campaign Name:</h3>
                <input value={campaign}
                       onChange={updateStateHandler(setCampaign)}
                       style={longInputStyle}
                />
            </Col>
            <Col lg={6} style={charDetails}>
                <Row>
                    <Col>
                        ...
                        <h3>Race:</h3>
                        <input value={race}
                               onChange={updateStateHandler(setRace)}
                               style={longInputStyle}
                        />
                    </Col>{" "}
                    <Col>
                        <h3>Background:</h3>
                        <input value={bg}
                               onChange={updateStateHandler(setBg)}
                               style={longInputStyle}
                        />
                        <h3>Alignment:</h3>
                        <DropdownButton
                        ...
                        >
                            {alignments.map((align) => <Dropdown.Item key={align.label}
                                                                      eventKey={align.label}>{align.label}</Dropdown.Item>)}
                        </DropdownButton>
                    </Col>
                </Row>
            </Col>
            <Col lg={2} style={charDetails}>
                <h3>EXP:</h3>
                <input value={exp}
                       onChange={updateStateHandler(setExp)} style={expInputStyle}/>
            </Col>
        </Row>
    );
};
